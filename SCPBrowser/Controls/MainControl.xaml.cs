    ...
    public partial class MainControl : UserControl
    {
        ...
        private async System.Threading.Tasks.Task LoadGoEnrichmentAsync()
        {
            ...
            try
            {
                ...
                double pValueCutoff = Settings.Default.GOPValueCutoff;
                int minOverlap = Settings.Default.GOMinimumOverlap;
                if (_databaseService != null)
                {
                    var pStr = await _databaseService.GetSettingAsync("GOPValueCutoff");
                    var mStr = await _databaseService.GetSettingAsync("GOMinimumOverlap");
                    if (pStr != null && double.TryParse(pStr, out var p)) pValueCutoff = p;
                    if (mStr != null && int.TryParse(mStr, out var m)) minOverlap = m;
                }
