...
namespace TransmutationLearning
{
    ...
    public class FeatureSelectionService
    {
            ...
            FeatureSelectionCriteria criteria)
        {
            ...
            // Build run -> cell type mapping from retained cells
            var runToCellType = filteredData.RetainedCells.ToDictionary(c => c.Run, c => c.Labels);
            
            // Get cell types present in retained data
            var cellTypes = filteredData.RetainedCells
                .Select(c => c.Labels)
                .Distinct()
                .OrderBy(c => c)
                .ToList();
