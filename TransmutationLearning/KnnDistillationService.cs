...
namespace TransmutationLearning
{
    ...
    public class KnnDistillationService
    {
            ...
            int protectionRank = 2)
        {
            ...
            var (featureMatrix, runOrder, proteinOrder) = BuildFeatureMatrix(retainedCells, proteinMatrix);
            ...
            Dictionary<string, Dictionary<string, double>> proteinMatrix)
        {
            ...
            var proteinOrder = proteinMatrix.Keys.OrderBy(p => p).ToList();
            ...
            int protectionRank)
        {
            ...
            // Sensitivity controls how much the prior influences voting:
            // 0 = pure kNN (no prior influence)
            // 1 = strong prior (expected types get up to 2x weight)
            ...
            foreach (var kvp in votes)
            {
                ...
                double rawVote = kvp.Value;

                // Prior bonus: types higher in expected list get bonus
                double priorScore = priorWeights.TryGetValue(label, out var pw) ? pw / maxPrior : 0.1;
                double multiplier = 1.0 + (sensitivity * priorScore); // Ranges from 1.0 to 1.0+sensitivity

                weightedVotes[label] = rawVote * multiplier;
            }

            // Find winning label
            double totalWeightedVotes = weightedVotes.Values.Sum();
            ...
            double confidence = winnerVotes / totalWeightedVotes;
